...
void loop(){
	...
	if (rf95.available()){
		...
		if (rf95.recv(buf, &len)){
			...
			uint16_t messageChunk = (((uint16_t) buf[0]) << 8) | buf[1];
			uint16_t messageType = Field(MSG_TYPE_SIZE).extract(&messageChunk);

			Message receivedMessage(messageType);
			receivedMessage.setMessage(buf);
			receivedMessage.readMessage();

			for (unsigned i = 0; i < receivedMessage.getFieldCount(); i++){
				Serial.print("Field : ");
				Serial.println(receivedMessage.getFields()[i].getValue());
			}
			// Send a reply
